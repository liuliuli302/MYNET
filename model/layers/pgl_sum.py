...
class SelfAttention(nn.Module):
    def __init__(
        self, input_size=1024, output_size=1024, freq=10000, heads=1, pos_enc=None
    ):
        ...
        if pos_enc is not None:
            ...
            assert (
                pos_enc in self.permitted_encodings
            ), f"Supported encodings: {*self.permitted_encodings,}"
...
class MultiAttention(nn.Module):
    def __init__(
        ...
        if self.fusion is not None:
            ...
            assert (
                self.fusion in self.permitted_fusions
            ), f"Fusion method must be: {*self.permitted_fusions,}"
            ...
            for segment in range(self.num_segments):
                ...
                if self.fusion == "add":
                    ...
                    weighted_value[left_pos:right_pos] = torch.max(
                        weighted_value[left_pos:right_pos].clone(), weighted_local_value
                    )
